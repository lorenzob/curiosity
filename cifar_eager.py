...
def loss(logits, labels, batch_data=None, training=True):
  ...
  if training:
    ...
    if curiosity:
      ...
      ### per_sample = np.random.shuffle(per_sample)  # TEMP TEMP TEMP TEMP TEMP
      ##per_sample = tf.random.shuffle
...
def compute_accuracy(logits, labels):
  ...
  #labels = tf.cast(labels, tf.uint32)
  batch_size = int(logits.shape[0])
  #print(predictions, "---", labels, tf.argmax(labels, 1))

  return tf.reduce_sum(
      tf.cast(tf.equal(predictions, tf.argmax(labels,1)), dtype=tf.float32)) / batch_size
...
def test(model, _, epoch):
  ...
  for batch in range(1000):
    ...
    accuracy(
        ...
        tf.cast(tf.argmax(labels,1), tf.int64))
...
def create_model():
  ...
  '''
  conv1 = keras.layers.Conv2D(64, 5,
                                 padding='same',
                                 activation=tf.nn.relu,
                                 kernel_initializer=tf.initializers.variance_scaling,
                                 kernel_regularizer=keras.regularizers.l2(l=0.001))
  max_pool2d = keras.layers.MaxPooling2D((3, 3), (2, 2), padding='same')
  max_norm = keras.layers.BatchNormalization()
  conv2 = keras.layers.Conv2D(64, 5,
                                 padding='same',
                                 activation=tf.nn.relu,
                                 kernel_initializer=tf.initializers.variance_scaling,
                                 kernel_regularizer=keras.regularizers.l2(l=0.001))
  flatten = keras.layers.Flatten()
  fc1 = keras.layers.Dense(750, activation=tf.nn.relu,
                              kernel_initializer=tf.initializers.variance_scaling,
                              kernel_regularizer=keras.regularizers.l2(l=0.001))
  dropout = keras.layers.Dropout(0.5)
  fc2 = keras.layers.Dense(10)
  softmax = keras.layers.Softmax()

  model = Model(inputs=conv1, outputs=softmax)
  '''
...
def define_mnist_eager_flags():
  ...
  flags.DEFINE_string(
      name='output_dir', short_name='od', default="logs16",
      help=flags_core.help_wrap('Directory to write TensorBoard summaries'))

  flags.DEFINE_float(name='learning_rate', short_name='lr', default=0.001,
...
def main(_):
  ...
  #color = ['b', 'r', 'g', 'm']
  #for ci, cr in enumerate([0.2, 0.3, 0.4, 0.5]):
  for ci, cr in enumerate([0.33]):
    curiosity = False #ci % 2 != 0
    curiosity_ratio = cr
