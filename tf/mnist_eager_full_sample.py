...
def loss(logits, labels, batch_data=None, training=True):
    ...
    if training:
        ...
        if curiosity:
            ...
            if classic_full_sample:
                retry_idx = np.random.choice(mnist_indexes, size=k, replace=False)
...
def main(_):
    global curiosity, curiosity_ratio, BATCH_SIZE, data_gen, test_gen, indexes, classic_full_sample
    ...
    for ci, cr in enumerate([0.25, 0.25, 0.25]):
        #curiosity = ci % 2 != 0
        #curiosity = True
        curiosity = ci == 0
        curiosity_ratio = cr

        classic_full_sample = ci == 2
        print("**********************")
        print("ci, cr", ci, cr)
        print("curiosity", curiosity)
        print("classic_full_sample", classic_full_sample)
